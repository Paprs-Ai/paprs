  ...
  return (
    ...
    >
      ...
      <div className="text-center max-w-4xl w-full mx-auto flex flex-col items-center mb-6">
        <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full border border-black/15 bg-zinc-100 font-mono text-[9px] font-bold uppercase tracking-widest text-black mb-2">
        ...
        <p className="font-sans text-xs md:text-sm text-zinc-600 font-medium mt-2 max-w-lg text-center">
          Zero risk to start. Lock in launch rates for life, with our free centralized vault included for every tier.
        </p>

        {/* Monthly / Annual Billing Toggle — Centered */}
        <div className="mt-4 flex items-center justify-center gap-2 bg-zinc-100 p-1 rounded-full border border-black/10">
          <button
          ...
          <button
            ...
            className={`px-4 py-1.5 rounded-full font-mono text-[11px] font-bold transition-all flex items-center gap-1.5 ${
              isAnnual
                ? "bg-black text-white shadow-xs"
                : "text-zinc-600 hover:text-black"
            }`}
          >
            Annual
            <span className="bg-white text-black text-[8px] px-1.5 py-0.5 rounded-full uppercase tracking-wider font-extrabold border border-black/10">
              Save ~20%
            </span>
          </button>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 w-full items-stretch justify-center">
        {tiers.map((tier) => {
          ...
          return (
            <div
              key={tier.id}
              className={`relative flex flex-col justify-between rounded-2xl p-4 md:p-5 transition-all duration-300 ${
                tier.highlight
                  ? "bg-black text-white border-2 border-black shadow-xl"
                  : "bg-white text-black border border-zinc-300 hover:border-black/50"
              }`}
            >
              {/* Badge Centered at Top */}
              {tier.badge && (
                <div
                  className={`absolute -top-3 left-1/2 -translate-x-1/2 px-2.5 py-0.5 rounded-full font-mono text-[8px] font-extrabold uppercase tracking-widest shadow-xs ${
                    tier.highlight
              ...
              <div>
                ...
                <p
                  className={`text-[11px] font-sans mb-4 min-h-[28px] leading-tight ${
                ...
                <div className="mb-4 pb-4 border-b border-zinc-200/20">
                  <div className="flex items-baseline gap-1">
                    <span className="font-syne text-3xl font-extrabold tracking-tight">
              ...
              <div>
                <a
                  href="#cta"
                  className={`w-full py-2.5 px-3 rounded-full font-syne text-[11px] font-bold uppercase tracking-wider transition-all duration-300 flex items-center justify-center text-center shadow-xs hover:scale-[1.02] ${
                    tier.highlight
      ...
      {/* Feature comparison highlights banner — Centered */}
      <div className="mt-6 w-full bg-zinc-50 rounded-2xl p-4 border border-zinc-200 grid grid-cols-1 md:grid-cols-3 gap-4">
